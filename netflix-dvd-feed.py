...
""" % ( g_cfg.main_name, g_cfg.main_url_base, 
        g_cfg.main_rss_base, g_cfg.main_url_base )
...
def write_feed(script_dir, feed_items):
    ...
    index = 0
    do_move = False
    temp_fname = os.path.join(script_dir, g_cfg.main_rss_base + '.temp.xml')
    dest_fname = os.path.join(script_dir, g_cfg.main_rss_base + '.xml')
...
if __name__=='__main__':
    ...
    if os.path.exists(os.path.join(script_dir, g_cfg.main_logfile)):
        with codecs.open(os.path.join(script_dir, g_cfg.main_logfile), 'r', 'utf-8') as f:
            lines = f.readlines()
    else:
        lines = []
    ...
    lines.insert(0, u"%s %3.0fs %s\n" % (time.strftime('%Y-%m-%d, %H:%M', time.localtime()),
                                         ...
                                         status))
    with codecs.open(os.path.join(script_dir, g_cfg.main_logfile), 'w', 'utf-8') as f:
        f.writelines(lines)
